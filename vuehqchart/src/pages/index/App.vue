  ...
  <div class="container">
    <div class="topWrap clearfix">
      <div class="stockInfo">
        ...
        <div class="codeInfo">
          <span class="code">{{GlobalSymbol}}</span>
          <i class="iconfont" @click="ShowSpell"></i>
          <div class="editSymbol" v-if="SpellStockShow.Input">
            <input v-focus maxlength="8" type="text" v-model="inputSymbolStr" />
            <i class="iconfont icon-close" @click="HideSpell"></i>
          </div>
          <ul class="symbolList" v-if="SpellStockShow.List">
            <li
              v-for="(currentStock,index) in SearchSymbols"
              :key="index"
              @click="ChangeStock(currentStock.symbol)"
            >
              <span class="symbol">{{currentStock.symbol}}</span>
      ...
      <div class="priceInfo">
        ...
        <p class="priceCurrent">
          <span class="riseNum" :class="pageDataColor.riseClass">{{pageData.get('rise')}}</span>
          <span class="risePrecent" :class="pageDataColor.riseClass">{{pageData.get('riseRate')}}</span>
        </p>
      </div>
      <div class="otherInfo">
        <ul>
          <li>
            <div class="high">
              高
              <span :class="pageDataColor.highClass">{{pageData.get('high')}}</span>
            </div>
            <div class="low">
              低
              <span :class="pageDataColor.lowClass">{{pageData.get('low')}}</span>
            </div>
          </li>
          <li>
            <div class="open">
              开
              <span :class="pageDataColor.openClass">{{pageData.get('open')}}</span>
            </div>
            <div class="change">
              换
              <span :class="pageDataColor.exchangeClass">{{pageData.get('exchange')}}</span>
            </div>
          </li>
          <li>
            <div class="num">
              额
              <span>{{pageData.get('amount')}}</span>
            </div>
            <div class="totalValue">
              量
              <span>{{pageData.get('vol')}}</span>
            </div>
          </li>
          <li>
            <div class="pe">
              PE
              <span>{{pageData.get('pe')}}</span>
            </div>
            <div class="eps">
              EPS
              <span>{{pageData.get('eps')}}</span>
            </div>
          </li>
          <li>
            <div class="marketV">
              总市值
              <span>{{pageData.get('marketV')}}</span>
            </div>
            <div class="scrollEPS">
              滚动EPS
              <span>{{pageData.get('scrollEPS')}}</span>
            </div>
          </li>
          <li>
            <div class="flowMarketV">
              流通市值
              <span>{{pageData.get('flowMarketV')}}</span>
            </div>
            <div class="roe">
              ROE
              <span :class="pageDataColor.roeClass">{{pageData.get('roe')}}</span>
            </div>
          </li>
          <li>
            <div class="pb">
              PB
              <span>{{pageData.get('pb')}}</span>
            </div>
            <div class="amplitude">
              振幅
              <span>{{pageData.get('amplitude')}}</span>
            </div>
          </li>
        </ul>
      </div>
    ...
    <div class="kLineSetWrap" id="kLineSetWrap">
      ...
      <div
        class="item"
        v-for="(menuOne,index) in MenuData"
        :key="index"
        @click="openMenuTwo(index)"
      ...
      >
        ...
        <ul class="menuTwo" v-show="MenuOneIndex == index ? true:false">
          <li
            v-for="(name,ind) in menuOne.Children"
            :class="{active:menuOne.ActiveIndex == ind}"
            :key="ind"
    ...
    <div class="chartDataWrap clearfix">
      ...
      <div class="leftChart">
        <div
          id="minuteChartID"
          ref="minuteChartID"
          style="width:1104px;height:590px;position: relative;"
          v-show="IsChartShow.MinuteChartShow"
        ></div>
        <div
          id="kLine"
          ref="kLine"
          style="width:1104px;height:558px;position: relative;"
          v-show="IsChartShow.KlineChartShow"
        ></div>
      ...
      <div class="rightData">
        ...
        <div class="firstLine">
          委比：
          <span :class="bookData.BookRate.ClassName">{{bookData.BookRate.Text}}</span>委差：
          <span :class="bookData.BookDiffer.ClassName">{{bookData.BookDiffer.Text}}</span>
        </div>
        <ul class="sellFive">
...
export default {
  ...
  methods: {
    ...
    ChangeBottomIndex(indexName, ind) {
      //切换图形底部指标
      if (
        this.IndexBottomData.type == "history" &&
        this.JSEnvironment.HistoryChart != null
      ) {
        this.JSEnvironment.HistoryChart.ChangeIndex(1, indexName);
        this.BottomIndexForHistory = ind;
      } else if (
        this.IndexBottomData.type == "minute" &&
        this.JSEnvironment.MinuteChart != null
      ) {
    ...
    ChangeCurrentCoordinateType(index) {
      if (this.JSEnvironment.HistoryChart != null) {
        this.JSEnvironment.HistoryChart.JSChartContainer.ChangeCoordinateType(
          index
        );
      }
    },
    ...
    showTools() {
      ...
      if (this.JSEnvironment.HistoryChart != null) {
        ...
        if (this.ExtendChart == null) {
          this.DrawTools = chart.JSChartContainer.GetExtendChartByClassName(
            "DrawToolsButton"
          );
          var drawTools = this.DrawTools;
          var option = {
            Name: "画图工具",
            Top: frame.ChartBorder.Top,
            IsAutoIndent: 1
          };
          this.ExtendChart = chart.JSChartContainer.CreateExtendChart(
            option.Name,
            option
          ); //创建扩展图形
    ...
    openMenuTwo(ind) {
      ...
      if (this.IsChartShow.MinuteChartShow) {
        //分时图
    ...
    changeKLineChart(currentData, name, ind) {
      //参数：一级菜单名称，二级菜单内容，二级菜单下标
    ...
    updateMinuteChart(count) {
      ...
      if (this.JSEnvironment.MinuteChart == null) {
        this.JSEnvironment.MinuteChart = jsChart.jsChartInit(
          this.$refs.minuteChartID
        ); // 初始化走势图
        jsChart.jsChartStyle(this.JSEnvironment.MinuteOption);
        this.JSEnvironment.MinuteChart.SetOption(
          this.JSEnvironment.MinuteOption
        );
      } else {
    ...
    UpdateMain(id, arySymbol, dataType, jsStockIntans) {
      //获得股票基础数据
      ...
      var riseFallPrice = read.Get(
        symbol,
        jsStock.STOCK_FIELD_NAME.RISE_FALL_PRICE
      );
      ...
      var excahngerate = read.Get(
        symbol,
        jsStock.STOCK_FIELD_NAME.EXCHANGE_RATE
      );
      ...
      var flowMarketV = read.Get(
        symbol,
        jsStock.STOCK_FIELD_NAME.FLOW_MARKET_VALUE
      );
    ...
    updateHistoryChart(period) {
      ...
      if (this.JSEnvironment.HistoryChart == null) {
        ...
        this.JSEnvironment.HistoryChart.SetOption(
          this.JSEnvironment.HistoryOption
        );
      } else {
    ...
    CaluChartWidthHeight() {
      ...
      var chartHeight =
        height -
        $(".topWrap").outerHeight(true) -
        $(".navBarWrap").outerHeight(true) -
        $(".kLineSetWrap").outerHeight(true) -
        $(".indexList").outerHeight(true) -
        5;
      var chartWidth = $(document).width() - 2;
  ...
  mounted() {
    ...
    document.addEventListener("click", e => {
      if (
        !$("#kLineSetWrap").is(e.target) &&
        $("#kLineSetWrap").has(e.target).length === 0
      )
        this.MenuOneIndex = 999;
    });
